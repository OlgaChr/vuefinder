<template>
  <div class="relative flex-auto flex flex-col overflow-hidden">
    ...
    <div
        ...
        class="h-full w-full text-xs vf-selector-area min-h-[550px] max-h-[80vh] overflow-auto p-1 z-0"
        ref="selectorArea">
      ...
      <div draggable="true"
          ...
          <div>
            <div class="relative">
              ...
              <img class="lazy h-10 md:h-12 m-auto" v-else-if="(item.mime_type ?? '').startsWith('image')" :data-src="item.imageUrl"  :alt="item.basename">
...
const getItems = (sorted = true) => {
  ...
  files = files.map(file => ({...file, imageUrl: getImageUrl(adapter.value, file.path)}))
